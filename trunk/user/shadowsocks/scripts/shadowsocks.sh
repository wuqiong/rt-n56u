...
get_ipt_ext(){
	if [ "$ss_lower_port_only" = "1" ]; then
		...
		echo '-e "-m multiport --dports 53,80,443,1935,8080"'
	fi
}
...
func_start_ss_rules(){
	...
	#sh -c "ss-rules -s $ss_server -l $ss_local_port $(get_wan_bp_list) -W $wan_fw_list -w $wan_fw_ips -d "RETURN" $(get_ipt_ext) $(get_arg_out) $(get_arg_udp)"
	return $?
}
