 ...
 */
int tcp_may_update_window(const struct tcp_sock *tp,
...
static int tcp_ack(struct sock *sk, struct sk_buff *skb, int flag)
{
	...
	newly_acked_sacked = tp->packets_out;
...
static void tcp_data_queue_ofo(struct sock *sk, struct sk_buff *skb)
{
	...
	if (seq == TCP_SKB_CB(skb1)->end_seq) {
		 ...
		 */
		if (!tp->mpc && skb->len <= skb_tailroom(skb1) && !skb_cloned(skb1) && !tcp_hdr(skb)->fin) {
			NET_INC_STATS_BH(sock_net(sk),
				  ...
				  const struct tcphdr *th, int syn_inerr)
{
	...
	if (tcp_fast_parse_options(skb, th, tp, &hash_location) && tp->rx_opt.saw_tstamp &&
